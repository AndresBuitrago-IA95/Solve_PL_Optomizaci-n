...
import { Maximize2, Layers, Compass, HelpCircle, Eye } from 'lucide-react';
...
export default function MetodoGrafico({ config, result }: MetodoGraficoProps) {
  ...
  // Sutherland-Hodgman clipping algorithm details
  ...
  // Scale limits nicely on multiples of 5 or 10
  ...
  // Sutherland-Hodgman region clip calculation
  ...
  // Clip against each problem constraint
  constraints.forEach(c => {
  ...
  useEffect(() => {
    ...
    // 1. Clear with deep dark cybertech background
    ctx.fillStyle = '#090909';
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = '#101010';
    ctx.fillRect(pad, pad, W - 2 * pad, H - 2 * pad);

    // 2. Draw minor Grid Lines in high contrast dark theme
    ...
    // 3. Draw Feasible Region (glowing semi-transparent neon-lime)
    ...
    // 4. Draw primary axes (sleek silver-gray)
    ...
    // Axes Ticks & Labels in Monospace
    ...
    ctx.fillText('X₁', W - pad + 25, H - pad + 4);
    ctx.fillText('X₂', pad, pad - 26);

    // 5. Draw constraints lines with standard color sequence
    ...
    if (result.solution && result.solution.variables) {
      const x1val = result.solution.variables.X1 ? result.solution.variables.X1.toDecimal() : 0;
      const x2val = result.solution.variables.X2 ? result.solution.variables.X2.toDecimal() : 0;
      const zOpt = result.solution.z ? result.solution.z.toDecimal() : 0;

      // Draw dashed purple Iso-Z line
      ...
      // Smart placement for coordinate flag
  ...
  if (config.numVars !== 2) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-[#262626] rounded-xl bg-[#0F0F0F]">
        <AlertCircleIcon className="w-12 h-12 text-zinc-650 mb-3" />
        ...
        <p className="text-zinc-500 text-xs max-w-sm font-sans">
          El método gráfico está limitado a representaciones de 2 variables de decisión (X₁ y X₂). Su modelo actual utiliza {config.numVars} variables.
        </p>
      </div>
    );
  }
      ...
      <div className="lg:col-span-8 bg-[#0F0F0F] border border-[#262626] rounded-xl p-6 shadow-sm flex flex-col items-center overflow-hidden" ref={containerRef}>
        <div className="w-full flex items-center justify-between mb-4 border-b border-[#202020] pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-white font-mono text-sm uppercase">Plano de Soluciones Factibles</h3>
          </div>
          <span className="text-[10px] bg-[#1C1C1C] px-2.5 py-1 rounded text-[#00FF9C] font-semibold border border-[#262626] font-mono">
            Escala: {roundedStep}u / cuadr
          </span>
        </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-[#0F0F0F] border border-[#262626] rounded-xl p-5 shadow-sm space-y-4">
          <h4 className="font-bold text-white text-xs uppercase font-mono tracking-wider flex items-center gap-2">
            <Layers className="w-4 h-4 text-[#00FF9C]" /> Convención de Restricciones
          </h4>
          <div className="space-y-3">
            {constraints.map((c, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center justify-between py-1.5 border-b border-[#1E1E1E] last:border-b-0">
            ...
            {result.solution && result.solution.variables && (
              <div className="flex items-center justify-between py-1.5 border-b border-[#1E1E1E] last:border-b-0">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-bold text-[#BD93F9] text-xs font-mono">Z óptima (Iso-Z)</span>
                </div>
                <span className="font-mono text-[#BD93F9] text-xs font-semibold">
                  {c1}X₁ + {c2}X₂ = {result.solution.z?.toDecimal().toFixed(2).replace(/\.00$/, '')}
                </span>
              </div>
            )}
            <div className="flex items-center justify-between py-1.5 border-b border-[#1E1E1E] last:border-b-0">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-emerald-400 text-xs font-mono">Polígono de Factibilidad</span>
              </div>
              <span className="text-zinc-550 text-[10px] text-right">Región admisible</span>
            </div>
          </div>
        </div>
        ...
        {result.solution && result.solution.variables && (
          <div className="bg-[#0D0D0D] border border-[#BD93F9]/20 rounded-xl p-5 shadow-lg text-zinc-100 space-y-4">
            <h4 className="font-bold text-white text-xs uppercase tracking-wider font-mono flex items-center gap-2">
              <Eye className="w-4 h-4 text-[#00FF9C]" /> Vértice Óptimo Seleccionado
            </h4>
            <div className="space-y-3.5 font-mono">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-[#00FF9C]">
                  ...
                  <span className="text-xs text-zinc-550 font-normal ml-2 font-sans">
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div className="bg-[#121212] border border-[#202020] p-2.5 rounded">
                  <span className="text-zinc-555 text-[9px] uppercase font-bold tracking-wider font-sans">Eje X₁</span>
                  <p className="text-sm font-bold text-zinc-200">
                    {result.solution.variables.X1?.toString()}
                  </p>
                </div>
                <div className="bg-[#121212] border border-[#202020] p-2.5 rounded">
                  <span className="text-zinc-555 text-[9px] uppercase font-bold tracking-wider font-sans">Eje X₂</span>
                  <p className="text-sm font-bold text-zinc-200">
                    {result.solution.variables.X2?.toString()}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-[#0F0F0F] border border-[#262626] rounded-xl p-5 text-zinc-450 text-xs leading-relaxed space-y-2 font-sans">
          ...
          <p className="text-zinc-200 font-mono"><strong>¿Cómo funciona el método gráfico?</strong></p>
          <p>Toda restricción lineal define un semiplano. Al interceptar todos los semiplanos con la restricción de no negatividad, se forma un polígono convexo llamado **región factible**.</p>
          <p>La solución óptima siempre ocurrirá en algunos de los vértices (esquinas) de este polígono. El software evalúa todos los puntos y proyecta la línea de nivel óptimo (Iso-Z) que maximiza o minimiza la meta.</p>
        </div>
      </div>
    </div>
  );
